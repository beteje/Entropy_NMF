%function [W_final,H_final,tol,err,err_final] = FuzzyEn_Mean_NMF(Y,W,H,m,p,r,eta_w,eta_h)
...
itr         = 50;          % Maximum number of iterations to try
tol         = zeros(1,itr); % Initialize the tolerance vector
err         = zeros(1,itr); % Initialize the error vector
err_final   = norm(CFuzzyEn_init);  % Set the current error value
W_final     = W;
H_final     = H;
...
for i=1:itr
    ...
    H_Grad = FuzzyEn_H_Update_Mean_mex(Y,W,H,m,p,r_col);
...
%     err(i) = norm(Y-W*H);
    ...
    fprintf('Iteration: %d \t Tolerance: %d \t Error: %d \t CFuzzyEn: %d\t %d\t %d\t %d\t %d\n',i,tol(i),err(i),CFuzzyEn_YWH(i,:));
%     if tol(i)<eps
%         break;
%     end  
